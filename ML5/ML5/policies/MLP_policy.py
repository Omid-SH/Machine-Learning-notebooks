    ...
    def get_action(self, obs):
        ...
        with torch.no_grad() :
            if self.discrete :
                ...
                dis = self.model(obs) + torch.mul(torch.exp(self.logstd), torch.randn(len(obs), self.ac_dim))
...
class MLPPolicySL(MLPPolicy):
    ...
    def update(self, observations, actions):
        ...
        if self.discrete:
            acts_prediction = torch.nn.Softmax(dim=1)(self.model(observations))
            loss= self.loss_fn(acts_prediction, torch.LongTensor(actions))


        else:
            acts_prediction = self.model(observations) + torch.mul(torch.exp(self.logstd), torch.randn(len(observations), self.ac_dim))
