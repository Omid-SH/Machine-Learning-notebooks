...
for i in range(100): ### 100 different set of weights.
    # define random a vector of random uniform weights with length of 4 and values between 1 and -1.
    ...
    for j in range(100):  ### 100 different run for set of
        ...
        while not done:
            cnt+=1
            # take the actions based on the random weights you've defined before.
            if (np.dot(weights, observation) >= 0) :
                action = 1
            else :
                action = 0

            # extract environment's new parameters with function called "step".
    ...
    # compare the average and best length and update the best length and best weights if it's necessary.
...
### Use this if runnig on ubuntu VS Code
import os
...
while not done:
    # take the actions via the best set of weights you observed.
    cnt+=1
    # take the actions based on the random weights you've defined before.
